node {
        ...
        stage('Deploy') {      
                ...
                sh 'docker run -d --rm -it --mount "type=bind,source=$(pwd),destination=/home" node:latest'
                sh 'docker container start node:latest'
                sh 'docker exec -it node:latest /bin/bash'
